...
The `generateContent()` function SHALL resolve the system prompt from the database via `getPrompt(env, chatId, 'content', lang)` instead of using a hardcoded constant. The function SHALL accept `chatId` and `lang` parameters for prompt resolution.
...
The `editContent()` function SHALL resolve the system prompt from the database via `getPrompt(env, chatId, 'edit', lang)` instead of using a hardcoded constant.

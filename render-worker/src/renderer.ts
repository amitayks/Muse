 ...
 *
 * Relocated from content-bot (`cloudflare-bot/src/services/tweet-card.ts`) into the
 * dedicated render-worker so its heavy deps (satori/resvg/bidi + WASM) stay out of the
 * bot Worker's cold-start path. The bot now invokes these via the RENDER service binding.
 */
...
async function loadFonts(env: RenderEnv): Promise<{ regular: ArrayBuffer; bold: ArrayBuffer }> {
...
async function resolveEmojiUrls(env: RenderEnv, text: string): Promise<Map<string, string>> {

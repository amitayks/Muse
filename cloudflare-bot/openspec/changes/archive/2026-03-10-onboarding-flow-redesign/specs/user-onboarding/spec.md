...
### Requirement: Welcome screen Learn More
**Reason**: The "Learn More" content (feature list) is now integrated directly into the welcome screen text. The separate Learn More screen and its callback (`onboard:learn`) are no longer needed.
**Migration**: Remove `renderLearnMore()` view function and `onboard:learn` callback handler. Move feature descriptions into the welcome screen text.

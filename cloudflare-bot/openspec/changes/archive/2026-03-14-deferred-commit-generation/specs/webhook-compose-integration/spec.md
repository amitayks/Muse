...
**Reason**: Content source data is now stored in `commit_events.source_data`. The `drafts` table no longer needs a `source_data` column. Drafts link to their source event via `event_id`.
**Migration**: Read source data from `commit_events` (via `draft.event_id`) instead of `drafts.source_data`. Since the `source_data` column was never deployed (added in undeployed `unified-compose-commits` change), no data migration is needed.

### Requirement: Webhook auto-generation stores source data
**Reason**: Webhooks no longer auto-generate. There is no draft created at webhook time, so there is no draft to store source data on. The source data is stored on `commit_events` instead.
**Migration**: Source data storage moved to `createCommitEvent` in the webhook handler.

### Requirement: Webhook notification buttons include Edit Compose option
**Reason**: The button layout has changed from `[✅ Approve] [✏️ Edit] / [👀 View] [🗑 Delete]` to `[⚡ Fast] [✏️ Edit]`. The Approve, View, and Delete buttons are no longer relevant at notification time because no draft exists yet.
**Migration**: Replaced by the new notification format in `github-integration` spec.

...
The system prompt for content generation SHALL instruct Grok to return `imagePrompt` as a structured JSON object matching the `ImagePromptData` schema, with each field meaningfully derived from the commit messages and file names.
